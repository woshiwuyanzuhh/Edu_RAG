...
| 优先级 | 项目 | 说明 |
|:--:|------|------|
| P3 | 前端测试缺失 | Vue 组件无单元测试 |
| P3 | E2E 测试 | 无端到端自动化测试 |
| P4 | CI/CD | GitHub Actions 仅 lint，缺自动测试/部署 |
| P4 | 类型导出 | 前端 API 类型手动维护，可考虑 OpenAPI 自动生成 |
| P5 | 多租户 | 当前单租户，未来可能需要知识库级隔离强化 |
